...
def test_parse(cleaned):
    ...
    if len(clauses) > 1:
        ...
        for clause in clauses:
            ...
            folder_m = re.search(r'(?:create|make|build)\s+(?:a\s+)?(?:new\s+)?(?:folder|directory|package)\s+(?:named\s+|called\s+)?[\'\"]?([a-zA-Z0-9_\-\./\\]+)[\'\"]?', clause, re.IGNORECASE)
            if not folder_m:
            ...
            # Extra specific check for 'containing exactly ...' or just 'containing'
            ...
            if is_folder and folder_m.group(1).endswith(exts):
                is_folder = False # It matched folder regex but it's a file
                file_m = folder_m
                is_file = True
            ...
            if is_folder:
                ...
                if last_file:
                    ...
                    valid = False
            elif any(v in clause.lower() for v in ["create", "write", "delete", "move", "rename"]):
                valid = False
