...
def test_parse(cleaned):
    clauses = re.split(r'\.\s+|\s+then\s+|,?\s*then\s+', cleaned, flags=re.IGNORECASE)
    if len(clauses) > 1:
        multi_plan = []
        valid = True
        ...
        for clause in clauses:
            ...
            folder_m = re.search(r'(?:create|make|build|put)\s+(?:a\s+)?(?:new\s+)?(?:folder|directory|package)\s+(?:named\s+|called\s+)?[\'\"]?([a-zA-Z0-9_\-\./\\]+)[\'\"]?', clause, re.IGNORECASE)
            if not folder_m:
                folder_m = re.search(r'(?:create|make|build)\s+([a-zA-Z0-9_\-/]+)(?:\s+on\s+desktop)?$', clause, re.IGNORECASE)
                
            file_m = re.search(r'(?:create|make|build|put)\s+(?:a\s+)?(?:file\s+)?(?:named\s+|called\s+)?[\'\"]?([a-zA-Z0-9_\-\./\\]+\.[a-zA-Z0-9]+)[\'\"]?(?:\s+(?:containing|with)\s+(?:exactly\s+)?(?:content\s+)?(.+))?', clause, re.IGNORECASE)
            if not file_m:
            ...
            # Special case for "put file.txt inside folder"
            ...
            if is_folder:
                ...
                if last_file:
                    ...
                    valid = False
                    print("read simple failed")
            
            if not matched:
                if any(v in clause.lower() for v in ["create", "write", "delete", "move", "rename"]):
                    valid = False
                    print(f"invalid keyword in unmatched clause: {clause}")
