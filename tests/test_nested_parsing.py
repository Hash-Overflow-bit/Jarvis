...
Then read the file back and verify its exact path and content."""
    ...
    assert Path(plan[0]["arguments"]["directory"]) == desktop / "nested_test"

    assert plan[1]["step"] == 2
    assert plan[1]["tool"] == "create_directory"
    assert Path(plan[1]["arguments"]["directory"]) == desktop / "nested_test" / "level1"

    assert plan[2]["step"] == 3
    assert plan[2]["tool"] == "create_directory"
    assert Path(plan[2]["arguments"]["directory"]) == desktop / "nested_test" / "level1" / "level2"

    assert plan[3]["step"] == 4
    assert plan[3]["tool"] == "write_file"
    assert Path(plan[3]["arguments"]["filepath"]) == desktop / "nested_test" / "level1" / "level2" / "notes.txt"
    assert plan[3]["arguments"]["content"] == "Nested filesystem test passed"

    assert plan[4]["step"] == 5
    assert plan[4]["tool"] == "read_file"
    assert Path(plan[4]["arguments"]["filepath"]) == desktop / "nested_test" / "level1" / "level2" / "notes.txt"
...
def test_nested_parsing_generalizations(prompt, expected_parts):
    ...
    from pathlib import Path
    desktop = settings.desktop_dir
    expected = desktop.joinpath(*expected_parts)
